...
export const updateCart = createAsyncThunk('/cart/update', async ({ userId, productId, quantity }) => {
  const response = await axios.put(`http://localhost:5000/api/shop/cart/updateCart`, { userId, productId, quantity });
  return response?.data || [];
});

export const deleteCart = createAsyncThunk('/cart/delete', async ({ userId, productId }) => {

  const response = await axios.delete(`http://localhost:5000/api/shop/cart/deleteCart/${userId}/${productId}`);
...
export const clearCart = createAsyncThunk(
  ...
  async (userId, thunkAPI) => {
    ...
    try {
      const response = await axios.delete(`http://localhost:5000/api/shop/cart/cartClear/${userId}`);
